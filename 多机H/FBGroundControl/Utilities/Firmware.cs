...
namespace MissionPlanner.Utilities
{
    public class Firmware
    {
        ...
        public bool UploadPX4(string filename, BoardDetect.boards board)
        {
            ...
            try
            {
                // check if we are seeing heartbeats检查如果我们看到heartbeats
                MainForm.comPort.BaseStream.Open();
                MainForm.comPort.giveComport = true;

                if (MainForm.comPort.getHeartBeat().Length > 0)
                {
                    MainForm.comPort.doReboot(true, false);
                    MainForm.comPort.Close();

                    //specific action for VRBRAIN4 board that needs to be manually disconnected before uploading
                    //对于vrbrain4板需要手动断开之前上传的具体行动
                ...
                {
                    MainForm.comPort.BaseStream.Close();
