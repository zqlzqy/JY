...
namespace MissionPlanner.Controls
{
    public class HUD : GLControl
    {
        ...
        public delegate void HUDSignal(float _linkqualitygcs);//状态栏：飞机飞行信号委托类型声明
        public static event HUDSignal HUDSignalHandler;//状态栏：飞机飞行信号基于委托的事件对象
        ...
        DateTime _datetime;
        string _mode = "Manual";
        string _runmode = "Manual";
        string _control_mode = "GNC";
        int _wpno = 0;
        ...
        [System.ComponentModel.Browsable(true), System.ComponentModel.Category("Values")]
        public float battery_voltage { get { return _battery_voltage; } set { if (_battery_voltage != value) { _battery_voltage = value; this.Invalidate(); } } }
        [System.ComponentModel.Browsable(true), System.ComponentModel.Category("Values")]
        public float servo_voltage { get { return _servo_voltage; } set { if (_servo_voltage != value) { _servo_voltage = value; this.Invalidate(); } } }
        [System.ComponentModel.Browsable(true), System.ComponentModel.Category("Values")]
        ...
        public string change_mode { get { return _control_mode; } set { if (_control_mode != value) { _control_mode = value; this.Invalidate(); } } }
        [System.ComponentModel.Browsable(true), System.ComponentModel.Category("Values")]

        public string runmode { get { return _runmode; } set { if (_runmode != value) { _runmode = value; this.Invalidate(); } } }  //失控模式
        [System.ComponentModel.Browsable(true), System.ComponentModel.Category("Values")]
        ...
        void doPaint(PaintEventArgs e)
        {
            ...
            try
            {
                ...
                //if (!connected)
                //{
                this._whiteBrush.Color = Color.LightCyan;
                ...
                if (bgon == true)
                {
                    ...
                    if (bg.Height != 0)
                    {
                        using (LinearGradientBrush linearBrush = new LinearGradientBrush(
                            bg, Color.DarkBlue, Color.LightSkyBlue, LinearGradientMode.Vertical))
                        {
                    ...
                    if (bg.Height != 0)
                    {
                        using (
                            LinearGradientBrush linearBrush = new LinearGradientBrush(
                                bg, Color.FromArgb(209, 135, 14), Color.FromArgb(166, 118, 7), LinearGradientMode.Vertical))
                        {
                            graphicsObject.FillRectangle(linearBrush, bg);
                        }
                    }
                ...
                if (displayrollpitch)
                {
                    ...
                    for (int a = -90; a <= 90; a += 5)
                    {
                        ...
                        if (a >= _pitch - 29 && a <= _pitch + 20)
                        {
                            ...
                            else
                            {
                                ...
                                // drawstring(graphicsObject, a.ToString(), new Font("Arial", 10), fontsize + 2, _whiteBrush, this.Width / 2 - lengthshort - 20 - halfwidth, this.Height / 2 + pitchoffset + a * every5deg - 8);
                            }
                        }
                    }
                    ...
                    graphicsObject.DrawArc(this._whitePen, arcrect, 180 + 30 + -_roll, 120); // 120画圆
                ...
                if (displayspeed)
                {
                    ...
                    arrow[1] = new Point(scrollbg.Width - 10 + hewith, -10);
                    arrow[2] = new Point(scrollbg.Width - 5 + hewith, 0);
                    arrow[3] = new Point(scrollbg.Width - 10 + hewith, 10);
                    ...
                    //if (_lowgroundspeed)
                    //{
                    //    drawstring(graphicsObject, HUDT.GS + _groundspeed.ToString("0.0"), font, fontsize, (SolidBrush)Brushes.Red, 1, scrollbg.Bottom + fontsize + 3 + 10);
                    //}
                    //else
                    //{
                    //    drawstring(graphicsObject, HUDT.GS + _groundspeed.ToString("0.0"), font, fontsize, _whiteBrush, 1, scrollbg.Bottom + fontsize + 3 + 10);
                    //}

                    //if (_control_mode.Equals("Loss_Connect"))
                    //{
                    //    drawstring(graphicsObject, _control_mode, font, fontsize, (SolidBrush)Brushes.Red, 1, scrollbg.Bottom + fontsize + 2 - 240);
                    //}
                    //else if (_control_mode.Equals("GNC"))
                    //{
                    //    drawstring(graphicsObject, _control_mode + " + " + _runmode, font, fontsize, _whiteBrush, 1, scrollbg.Bottom + fontsize + 2 - 240);
                    //}
                    //else
                    //{
                    //    drawstring(graphicsObject, _control_mode+"111", font, fontsize, _whiteBrush, 1, scrollbg.Bottom + fontsize + 2 - 240);
                    //}
                }
                // drawstring(graphicsObject, _control_mode + "111", font, fontsize, _whiteBrush, 1, scrollbg.Bottom + fontsize + 2 - 240);
                ...
                if (displayalt)
                {
                    ...
                    drawstring(graphicsObject, ((int)_alt).ToString("0"), font, 10, (SolidBrush)Brushes.AliceBlue, scrollbg.Left + size, -9);
                    graphicsObject.ResetTransform();
                    //修改右下角飞行模式显示位置 2017-8-31
                    // mode and wp dist and wp  _mode
                    drawstring(graphicsObject, _mode, font, fontsize, _whiteBrush, 5, scrollbg.Top + fontsize - 50);
                    if (_runmode == "正常飞行")
                    { _runmode = " "; }
                    drawstring(graphicsObject, _runmode, font, fontsize, _redBrush, scrollbg.Right - 150, scrollbg.Bottom + fontsize + 20);//失控模式显示位置
                    //去掉右下角距离 2017-8-31
                    // drawstring(graphicsObject, (int)_disttowp + ">" + _wpno, font, fontsize, _whiteBrush, scrollbg.Left - 30, scrollbg.Bottom + fontsize + 2 + 10);
                }

                if (displayconninfo)//118
                {
                    ...
                    //}
                    //if (HUDSignalHandler != null)
                    //{
                    //    HUDSignalHandler(_linkqualitygcs);
                    //}
                ...
                string batteryvolt = battery_voltage.ToString("0.00v");
                string servovolt = HUDT.Bat + _servo_voltage.ToString("0.00v");
                // drawstring(graphicsObject, "飞控电压" + batteryvolt, font, fontsize, _whiteBrush, fontsize - 10, this.Height - 40 - fontoffset);
                //                drawstring(graphicsObject, "舵机电压" + servovolt, font, fontsize, _whiteBrush, fontsize - 10, this.Height - 30 - fontoffset);
